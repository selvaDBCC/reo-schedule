...
function parseReoText(pageOneText, allText){
  ...
  // OCR-friendly fallbacks: Ctrl Code might be on a separate line in OCR output
  if(!e.ctrlCode){m=a.match(/Ctrl\s*Code:?\s*([A-Z0-9]{3,5})/i);if(m)e.ctrlCode=m[1].trim()}
  if(!e.shipDate){m=a.match(/Ship\s*Date:?\s*([\d\/]+)/i);if(m)e.shipDate=m[1].trim()}
  if(e.weight==null){m=a.match(/Wt:?\s*([\d.,]+)\s*T(?:onne)?/i);if(m){const v=parseReoNum(m[1]);if(!isNaN(v))e.weight=v}}
