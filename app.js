...
function initSupabase(){
  ...
  return sb}
let projects=[],entries=[],pendingFile=null,pendingMarkups=[],editingId=null,adminUnlocked=false;
...
async function doPdfExtract(file){
  ...
  m=t.match(/Wt:\s*([\d.,]+)\s*T/i);
  if(m){const raw=m[1].replace(/,/g,'.');const v=parseFloat(raw);if(!isNaN(v))e.weight=v}
  if(!e.weight){
    let allText=t;
    for(let i=2;i<=pdf.numPages;i++){try{const pp=await pdf.getPage(i);const tt=await pp.getTextContent();allText+=' '+tt.items.map(x=>x.str).join(' ')}catch(_){}}
    m=allText.match(/Total\s*Weight:\s*([\d.,]+)\s*Tonne/i);
    if(m){const raw=m[1].replace(/,/g,'.');const v=parseFloat(raw);if(!isNaN(v))e.weight=v}}
  return e}
  ...
  try{
    if(currentEntryType==='loose'){
      ...
      if(selectedOrderId){
        ...
        const up={schedule,entry_date:ed,supplier_delivery_date:supD,file_url:furl,file_name:fname,status:'Scheduled',total_weight:weight?parseFloat(weight):null,drawing_reference:drawing,comments:comments||entry.comments};
        ...
        const{data,error}=await sb.from('entries').insert({project,level:level||null,area:area||null,schedule,status:'Scheduled',entry_date:ed,comments:comments||null,file_url:furl,file_name:fname,entry_type:'scheduled',total_weight:weight?parseFloat(weight):null,drawing_reference:drawing,supplier_delivery_date:supD,markup_plans:mkNew.length?JSON.stringify(mkNew):null,unmatched:true}).select().single();
        if(error)throw error;
...
<td class="sched-td">${e.schedule?esc(e.schedule):'<span style="color:#ccc">—</span>'}</td>
...
<div style="display:flex;gap:8px;justify-content:flex-end;margin-top:14px"><button class="btn btn-sec btn-sm" onclick="closeOv('detailOv')">Cancel</button><button class="btn btn-sm" onclick="confirmAttach(${id})" id="att_btn" style="width:auto">Attach</button></div>`;
  ...
  if(ep){$('att_info').innerHTML='<div class="info-msg">Extracting...</div>';
    ep.then(ext=>{window._attExt=ext;
...
async function confirmAttach(id){
  ...
  try{const e=entries.find(x=>x.id===id),file=window._attFile;
    const furl=await uploadFile(file,e.project,e.level,e.area);
    const up={schedule:s,entry_date:sub,supplier_delivery_date:sd,file_url:furl,file_name:file.name,status:'Scheduled',total_weight:wt?parseFloat(wt):null,drawing_reference:dr||null};
...
<div style="display:flex;gap:8px;justify-content:flex-end;margin-top:14px"><button class="btn btn-sec btn-sm" onclick="closeOv('emailOv')">Cancel</button><button class="btn btn-purple btn-sm" onclick="sendEmail(${entryId},'${esc(auditContext)}')" style="width:auto">✉ Send</button></div>`;$('emailOv').classList.add('show')}
...
async function sendEmail(id,context){const to=$('em_to').value.trim(),sub=$('em_sub').value.trim(),body=$('em_body').value.trim();
  ...
  await auditLog({entry_id:id,action:'EMAIL_SENT',field_changed:context,new_value:'To: '+to+' — '+sub});closeOv('emailOv')}
...
function showAdminSub(s){['proj','program','fixers','audit'].forEach(t=>{
  ...
  if(s==='proj')renderAdminProj();if(s==='program')renderAdminProgram();if(s==='fixers')renderAdminFixers();if(s==='audit')loadAuditLog()}
...
/* ═══ ADMIN: STEEL FIXERS (placeholder) ═══ */
function renderAdminFixers(){
  $('adminFixers').innerHTML=`<div class="card"><div class="coming-soon"><div style="width:60px;height:60px;background:var(--accent-lt);border-radius:50%;display:inline-flex;align-items:center;justify-content:center;margin-bottom:14px"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--accent-dk)" stroke-width="2"><path d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z"/></svg></div><h3>Steel Fixers — Coming Soon</h3><p>Weight breakdown extraction and fixer allocation tools will be available in the next build.</p></div></div>`}
