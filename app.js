...
async function doPdfExtract(file){
  ...
  if(e.weight==null){m=allText.match(/Total\s*Weight:\s*([\d.,]+)\s*Tonne/i);if(m){const v=parseReoNum(m[1]);if(!isNaN(v))e.weight=v}}
  ...
  const bsIdx=allText.search(/BAR\s*SUMMARY/i);
  if(bsIdx>=0){
    const slice=allText.slice(bsIdx);
  ...
  const meshRe=/\b((?:SL|RL)\d+[A-Z]*)\s+([\d.,]+)\s+Each\s+Square\s+Mesh\s+(?:(?:SL|RL)\d+[A-Z]*\s+)?effective\s*area\s*([\d.,]+)\s*[xX]\s*([\d.,]+)\s*m/gi;
  let mm;while((mm=meshRe.exec(allText))!==null){
  ...
  let tr;while((tr=trenchRe.exec(allText))!==null){
    const qty=parseReoInt(tr[2]),a=parseReoDim(tr[3]),b=parseReoDim(tr[4]);
    if(!isNaN(qty)&&!isNaN(a)&&!isNaN(b)){trenchLm+=qty*Math.max(a,b);trenchFound=true}}
...
async function submitEntry(){
  ...
  try{
    if(currentEntryType==='loose'){
      ...
      const breakdown={bar_weight:ex.barWeight!=null?ex.barWeight:null,mesh_sqm:ex.meshSqm!=null?ex.meshSqm:null,trench_mesh_lm:ex.trenchLm!=null?ex.trenchLm:null};
      if(selectedOrderId){
...
<div style="display:flex;gap:8px;justify-content:flex-end;margin-top:14px"><button class="btn btn-sec btn-sm" onclick="closeOv('detailOv')">Cancel</button><button class="btn btn-sm" onclick="confirmAttach(${id})" id="att_btn" style="width:auto">Attach</button></div>`;
  ...
  if(ep){$('att_info').innerHTML='<div class="info-msg">Extracting...</div>';
    ep.then(ext=>{window._attExt=ext;
      ...
      if(ext.barWeight!=null)ff.push('Bars: '+ext.barWeight+'T');
      if(ext.meshSqm!=null)ff.push('Mesh: '+ext.meshSqm+'m²');
      if(ext.trenchLm!=null)ff.push('Trench: '+ext.trenchLm+'LM');
...
async function sendEmail(id,context){
  ...
  await auditLog({entry_id:id,action:'EMAIL_SENT',field_changed:context,new_value:'To: '+to+' — '+sub});
  closeOv('emailOv')}
...
function getSfFiltered(){
  ...
  list.sort((a,b)=>{let va=a[sfSort.col],vb=b[sfSort.col];
    if(sfSort.col==='our_delivery_date'){va=new Date(va||0);vb=new Date(vb||0);return sfSort.asc?va-vb:vb-va}
...
function renderSfTable(){
  ...
  let sumBar=0,sumMesh=0,sumTr=0;
  list.forEach(e=>{sumBar+=parseFloat(e.bar_weight)||0;sumMesh+=parseFloat(e.mesh_sqm)||0;sumTr+=parseFloat(e.trench_mesh_lm)||0});
...
<td class="proj-td" title="${esc(e.project)}">${esc(e.project)}</td>
<td>${esc(e.level||'—')}${e.split_reference?' <span style="font-size:10px;color:var(--accent-dk);font-weight:600">('+esc(e.split_reference)+')</span>':''}</td>
<td title="${esc(e.area||'')}">${esc(e.area||'—')}</td>
...
async function sfSave(field,id){
  ...
  const e=entries.find(x=>x.id===id);
  await sb.from('entries').update({[field]:nv}).eq('id',id);
  await auditLog({entry_id:id,action:'UPDATE',field_changed:field,old_value:String(e[field]||''),new_value:String(nv==null?'':nv)});
...
function exportSfCSV(){
  ...
  const h=['Project','Level','Area','Split','Schedule','Ordered Delivery','Bar Weight (T)','Mesh (m²)','Trench Mesh (LM)','File'];
  const rows=list.map(e=>[e.project,e.level||'',e.area||'',e.split_reference||'',e.schedule||'',e.our_delivery_date||'',e.bar_weight??'',e.mesh_sqm??'',e.trench_mesh_lm??'',e.file_name||'']);
  // Totals row
  let sumBar=0,sumMesh=0,sumTr=0;list.forEach(e=>{sumBar+=parseFloat(e.bar_weight)||0;sumMesh+=parseFloat(e.mesh_sqm)||0;sumTr+=parseFloat(e.trench_mesh_lm)||0});
  rows.push(['','','','','','TOTALS',sumBar.toFixed(3),sumMesh.toFixed(2),sumTr.toFixed(2),'']);
  const csv=[h,...rows].map(r=>r.map(c=>`"${String(c??'').replace(/"/g,'""')}"`).join(',')).join('\n');
...
function renderSite(){
  const fp=$('svProj').value,fl=$('svLevel').value,fa=$('svArea').value,fs=$('svStatus').value,fq=($('svSearch').value||'').toLowerCase().trim();
  let list=entries.slice();
  if(fp)list=list.filter(e=>e.project===fp);
  if(fl)list=list.filter(e=>e.level===fl);
  if(fa)list=list.filter(e=>e.area===fa);
  if(fs==='active')list=list.filter(e=>e.status!=='Delivered'&&e.status!=='Cancelled');
  else if(fs==='delivered')list=list.filter(e=>e.status==='Delivered');
  else if(fs==='cancelled')list=list.filter(e=>e.status==='Cancelled');
